...
def main():
    ...
    if config.G_INIT_MODEL:
        ...
        model_opr.load_model(model.G, config.G_INIT_MODEL, config.MODEL.DEVICE, strict=False, cpu=True)
    if config.D_INIT_MODEL:
        if rank <= 0:
            logger.info('[Initing] Discriminator')
        model_opr.load_model(model.D, config.D_INIT_MODEL, config.MODEL.DEVICE, strict=False, cpu=True)
    ...
    if config.CONTINUE_ITER:
        ...
        model_opr.load_model(model.G, G_model_path, device, strict=True, cpu=True)
        model_opr.load_model(model.D, D_model_path, device, strict=True, cpu=True)
