...
class MixDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        img_hr = cv2.imread(url_hr, cv2.IMREAD_GRAYSCALE)
        img_lr = cv2.imread(url_lr, cv2.IMREAD_GRAYSCALE)
        ...
        img_lr = np.transpose(img_lr, (2, 0, 1)).astype(np.float32)
        img_hr = np.transpose(img_hr, (2, 0, 1)).astype(np.float32)
